...
export const ProductDetail = () => {
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-6">
        <p className="text-text-secondary">Carregando detalhes...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="container mx-auto p-6">
        ...
        <Link to="/" className="text-primary-strong hover:text-primary-dark hover:underline">
          Voltar ao catálogo
        </Link>
      </div>
    );
  ...
  return (
    <div className="container mx-auto p-6">
      {info.image_url && (
        <div className="mb-6 overflow-hidden rounded-lg border border-text-secondary shadow-sm">
          <img src={info.image_url} alt={info.nome_produto} className="w-full h-64 object-cover" />
        </div>
      )}
      <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-text-primary">{info.nome_produto}</h1>
          <p className="text-sm text-text-secondary">Categoria: {info.categoria_produto || 'Sem categoria'}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-surface rounded-lg shadow-md p-6 border border-text-secondary">
          <h2 className="text-xl font-semibold mb-4 text-text-primary">Detalhes do Produto</h2>
        ...
        <div className="bg-surface rounded-lg shadow-md p-6 border border-text-secondary">
          <h2 className="text-xl font-semibold mb-4 text-text-primary">Métricas</h2>
          <div className="space-y-3 text-text-primary">
            <p>
      ...
      {product.comentarios && product.comentarios.length > 0 && (
        <div className="mt-6 bg-surface rounded-lg shadow-md p-6 border border-text-secondary">
          <h2 className="text-xl font-semibold mb-4 text-text-primary">Comentários dos Clientes</h2>
          ...
          <div className="space-y-4">
            {product.comentarios.map((comentario) => (
              <div key={comentario.id_avaliacao} className="border-b border-text-secondary pb-4 last:border-b-0 last:pb-0">
                ...
                {comentario.titulo_comentario && comentario.titulo_comentario !== "Sem título" && (
                  <h3 className="font-semibold text-text-primary mb-1">{comentario.titulo_comentario}</h3>
                )}
                <p className="text-text-primary">{comentario.comentario}</p>
              </div>
            ))}
          </div>
          ...
          {product.total_comments && product.comentarios.length < product.total_comments && (
            <button
              ...
              className="mt-6 px-4 py-2 bg-primary-strong text-white rounded hover:bg-primary-dark transition-colors disabled:opacity-50"
            >
          ...
          {showAllComments && product.total_comments && product.comentarios.length === product.total_comments && (
            <button
              onClick={handleToggleComments}
              className="mt-6 px-4 py-2 bg-primary-strong text-white rounded hover:bg-primary-dark transition-colors"
            >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-background rounded-lg shadow-lg p-6 max-w-sm mx-4">
            <h2 className="text-xl font-bold text-text-primary mb-2">Confirmar exclusão</h2>
            ...
            <div className="flex gap-3 justify-end">
              <button
                ...
                disabled={deleting}
                className="px-4 py-2 bg-surface text-text-primary rounded hover:bg-primary transition-colors disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
